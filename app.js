    ...
    function App() {
      ...
      const handleUpdateTranslateAndMakeEpub = async (item) => {
        ...
        setDownloadingUpdates(prev => ({ ...prev, [novelKey]: true }));
        try {
          await window.LibraryEngine.updateTranslateAndStage(item, {
            novelUpdateBadges,
            loadFullNovel,
            saveNovelToHistory
          }, {
      ...
      const handleCheckAllUpdates = async () => {
        if (!window.LibraryEngine) return;
        await window.LibraryEngine.checkAllUpdates(webImportHistory, {
          toast,
          loadFullNovel,
          onBatchStart: () => setIsBatchChecking(true),
          onBatchEnd: () => setIsBatchChecking(false),
          onNovelCheckingStart: (novelId) => setCheckingUpdates(prev => ({ ...prev, [novelId]: true })),
          onNovelCheckingEnd: (novelId) => setCheckingUpdates(prev => ({ ...prev, [novelId]: false })),
          onBadgeUpdate: (novelId, badgeData) => setNovelUpdateBadges(prev => ({ ...prev, [novelId]: badgeData }))
        });
      };
...
// (toast is defined above)
      ...
      const handleTranslateText = async (resume = false) => {
        const engine = window.TranslationLoopEngine || TranslationLoopEngine;
        if (engine?.Controller?.executeTranslateText) {
          return engine.Controller.executeTranslateText({
            inputText,
            resume,
            provider,
            enableStreaming,
      ...
      const handleTranslateEbook = async (chapters, resume = false, isEpubParam = false, originalZipParam = null) => {
        const engine = window.TranslationLoopEngine || TranslationLoopEngine;
        if (engine?.Controller?.executeTranslateEbook) {
          return engine.Controller.executeTranslateEbook({
            chapters,
            resume,
            isEpubParam,
            originalZipParam,

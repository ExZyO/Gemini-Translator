...
Output ONLY the glossary lines starting with a hyphen. Do not wrap in markdown code blocks or add conversational chatter.`;
      ...
      return h(React.Fragment, null,
        ...
        activeBookMenuNovel && (() => {
          const item = activeBookMenuNovel;
          ...
          return h('div', { className: 'modal-overlay', onClick: () => setActiveBookMenuNovel(null) },
            h('div', { className: 'modal-box', style: { maxWidth: 460, maxHeight: '85vh', display: 'flex', flexDirection: 'column' }, onClick: e => e.stopPropagation() },
              h('div', { className: 'modal-hd' },
                ...
                h('button', { type: 'button', className: 'icon-btn', style: { width: 30, height: 30 }, onClick: () => setActiveBookMenuNovel(null) }, '✕')
              ),
              h('div', { className: 'modal-bd custom-scrollbar', style: { overflowY: 'auto', padding: '14px 16px' } },
                menuRow('✏️', 'Rename Novel', 'Set a custom title for EPUB export, library, and future updates', () => {
                  setRenameModalNovel(item);
                  setNewNovelTitleInput(item.customTitle || getCustomTitle(item) || item.title || '');
                }),
                menuRow('📝', 'Edit in Ebook Studio', 'Edit chapters, prose, covers, and table of contents', async () => {
                  const full = await loadFullNovel(item);
                  if (!full) { toast('Novel data not found in local store.', 'error'); return; }
                  setActiveTab('studio');
                  setStudioSubTab('edit');
                  localStorage.setItem('activeTab', 'studio');
                ...
                menuRow('⭐', item.inSavedSpace ? 'Remove from Saved Space' : 'Save to Space', item.inSavedSpace ? 'Remove priority star bookmark' : 'Pin to personal Saved Space shelf', () => {
                  toggleNovelSavedSpace(item.id, item);
                ...
                menuRow('🗑️', 'Remove from Library', 'Delete novel and cached chapters from local library', () => {
                  confirmAction(`Remove "${item.title}" from the library?`, async () => {
                    await deleteNovelFromHistory(item.id);
                  });
                }, true)
              )
            )
        ...
        ongoingEpubModal && ongoingEpubModal.isOpen && h('div', {
          ...
          h('div', {
            ...
            h('button', {
              ...
              onClick: () => setOngoingEpubModal(null)
            }, '‹ Back'),
          ...
          h('div', {
            ...
            // 2. Automated Multi-Source & Extension Search
                  ...
                  if (window.NativeBridge && window.NativeBridge.chooseFolder) {
                    try {
                      ...
                      if (res && res.treeUri) {
                        ...
                        await updateNovelFolderRecord({ id: ongoingEpubModal.uuid, title: ongoingEpubModal.title }, res.treeUri, res.displayPath || res.treeUri);
                    ...
                    if (p && p.trim()) {
                      ...
                      setOngoingEpubModal(prev => prev ? { ...prev, folderOptions: fOpts } : null);
                      await updateNovelFolderRecord({ id: ongoingEpubModal.uuid, title: ongoingEpubModal.title }, '', p.trim());
            ...
            h('div', {
              ...
              ongoingEpubModal.sourceUrl && h('div', {
                ...
                h('button', {
                  ...
                  onClick: () => handleScanContinuationToc(ongoingEpubModal.sourceUrl, ongoingEpubModal.existingCount)
                }, ongoingEpubModal.isScanningToc ? 'Scanning…' : '🔄 Re-scan')
              ),

              // Source Switcher Tray (always visible if !sourceUrl or if toggled)
              (!ongoingEpubModal.sourceUrl || ongoingEpubModal.showSourceSwitcher) && h('div', {
                ...
                h('div', { style: { display: 'flex', gap: 8 } },
                  ...
                  h('button', {
                    ...
                    disabled: ongoingEpubModal.isSearchingSources || ongoingEpubModal.isFetching,
                    onClick: () => handleSearchContinuationSources(ongoingEpubModal.searchQuery || ongoingEpubModal.title)
                ...
                !ongoingEpubModal.isSearchingSources && ongoingEpubModal.continuationSources?.length > 0 && h('div', {
                  ...
                  ongoingEpubModal.continuationSources.slice(0, 8).map((srcItem, sIdx) => {
                    ...
                    return h('div', {
                      ...
                      onClick: () => handleSelectContinuationSource(srcItem)
                    },
                      h('div', {
                        style: {
                          width: 32,
                ...
                h('div', { style: { marginTop: 6 } },
                  ...
                  h('div', { style: { display: 'flex', gap: 8 } },
                    h('input', {
                      ...
                      onKeyDown: (e) => {
                        if (e.key === 'Enter' && ongoingEpubModal.sourceUrl?.trim()) {
        ...
        // ── SWIFTAUDIO FLOATING MINI-PLAYER ──
        (audioPlayerState?.currentBook) && h('div', {
        ...
        // ── SWIFTAUDIO FULL PLAYER MODAL & FULLSCREEN ──
        isFullPlayerOpen && audioPlayerState?.currentBook && h('div', {
          ...
          h('div', {
            ...
            h('div', {
              ...
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
                (() => {
                  const inLib = isAudiobookInLibrary(audioPlayerState.currentBook);
                  return h('button', {
                    type: 'button',
                    className: 'mini-btn ghost',
                    style: {
                    ...
                    onClick: () => {
                      if (inLib) {
                        removeAudiobookFromLibrary(audioPlayerState.currentBook);
                      } else {
                        saveAudiobookToLibrary(audioPlayerState.currentBook);
                      }
                    }
                  }, inLib ? '⭐ In Library' : '☆ Add to Library');
                })(),
              ...
              h('div', { style: { marginTop: 10 } },
                h('div', { style: { fontSize: 12, fontWeight: 700, color: 'var(--paper)', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
                  ...
                  h('button', {
                    ...
                    onClick: () => handleOpenAudioDownload(audioPlayerState.currentBook)
                  }, '📥 Download MP3s')
                ),
                        ...
                        h('button', {
                          ...
                          onClick: async (e) => {
                            ...
                            try {
                              ...
                              await window.SwiftAudioEngine.Downloader.downloadSingleTrack(t, audioPlayerState.currentBook, getNovelFolderOptions(audioPlayerState.currentBook));
        ...
        // ── SWIFTAUDIO BACKGROUND DOWNLOAD PROGRESS BAR ──
        audioDownloadModal?.active && audioDownloadModal?.isMinimized && h('div', {
          style: {
            position: 'fixed',
        ...
        // ── SWIFTAUDIO BATCH DOWNLOAD MODAL ──
        audioDownloadModal && !audioDownloadModal.isMinimized && h('div', {
          className: 'modal-overlay',
                        ...
                        h('button', {
                          ...
                          onClick: async (e) => {
                            ...
                            try {
                              ...
                              await window.SwiftAudioEngine.Downloader.downloadSingleTrack(track, audioDownloadModal.book, audioDownloadModal.folderOptions || getNovelFolderOptions(audioDownloadModal.book));
        ...
        sourcePluginsModalOpen && h('div', {
            ...
            h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 } },
              h('div', { style: { display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: 1 } },
                h('button', {
                  ...
                  onClick: () => setSourcePluginsModalOpen(false),
          ...
          h('div', {
            ...
            pluginSelectedTab === 'catalog' && h(React.Fragment, null,
              h('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } },
                h('input', {
                  type: 'text',
                  ...
                  list.slice(0, 100).map(p => {
                    ...
                    return h('div', {
                      ...
                      isInstalled ? h('button', {
                        type: 'button',
                        className: 'mini-btn danger',
                        style: { padding: '6px 14px', fontSize: 11.5, borderRadius: 6, flexShrink: 0 },
                        onClick: () => handleUninstallPlugin(p.id)
                      }, 'Uninstall') : h('button', {
                        type: 'button',
                        className: 'mini-btn',
